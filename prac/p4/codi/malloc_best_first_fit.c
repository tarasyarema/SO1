...
p_meta_data search_available_space(size_t size_bytes) {
    ...
    while (current) {
        if (current->available && current->size_bytes >= size_bytes) {
            ...
            if (current->size_bytes < best->size_bytes)
                best = current;
        }
...
void free(void *ptr)
{
    ...
    p_meta_data meta_previous = NULL;

    // Check if there's free space next
    if (meta->next != NULL) {
        meta_next = meta->next;

        // Append the next free space to the meta size
        if (meta_next->available) {
            meta->size_bytes += (meta_next->size_bytes + SIZE_META_DATA);
            meta->next = meta_next->next;
        }
    }

    // Check if there's free space before 
    if (meta->previous != NULL) {
        meta_previous = meta->previous;

        // Make the previous free space use the meta size
        if (meta_previous->available) {
            meta_previous->size_bytes += (meta->size_bytes + SIZE_META_DATA);
            meta_previous->next = meta->next;
        }
    }
